...
项目特有稳定覆盖写在`AGENTS.md`的专用覆盖区，原有长规则保存在项目原生规则区。模型策略不属于根覆盖；用户确认的新Worker运行策略由项目总览受管块唯一承载，并由固定脚本展示、保存和解析。它不会改变当前PM模型。初始化或升级后应把不含秘密的根入口纳入业务项目版本管理，否则未来无法可靠追溯规则由谁写入。
