...
旧版结果收件箱只按升级裁决回收，不再承担新任务回源；新任务以平台保存的Worker final为正式真值。

唤醒只表示PM应扫描，不证明Worker已经结束。若回调来源仍在运行且没有可读final，只对该来源调用一次`wait_threads(timeoutMs=30000)`，返回后再扫描全部登记Worker；不得循环或等待未回调Worker。再次扫描后仍在运行则保持`进行中`，不验收、不更新工作台；平台明确显示本轮已经结束且没有可读final时按第3节一次转为`已暂停`，然后结束当前回合。

## 1. 处理暂停

从Worker唤醒发现已结束任务后，先按工作台定位唯一Worker并读取该任务final，再只核对；PM回合扫尾只作缺失回调时的补漏：
...
有效暂停用`workbench.pause`一次提交`已暂停 + 唯一原因 + 恢复条件 + final入口`，原任务继续留在活动区。用户可以直接在原 Worker 对话补充决定、资源或任务内授权并继续，不必回 PM重复审批；变化影响主线、其他任务、共享对象或高风险边界时，Worker再向 PM返回最小摘要。
...
从Worker唤醒发现已结束任务后，先按工作台定位唯一Worker并读取该任务final，再定点核对；PM回合扫尾只作缺失回调时的补漏：
...
证据闭合时用`workbench.accept`一次提交验收与收敛。证据不足但可以在原范围内补齐时保持`进行中`，让原Worker补证或修复；只有需要新的业务决定、高风险授权、共享冲突处理或必要外部资源时才转`已暂停`。

PM处理完成结果只更新任务状态和验收结论，不建立“候选完成、待采纳、验证中、已消费”等额外业务状态。

## 3. 重复、冲突与线程恢复

同一Worker final入口已经写入工作台时只处理一次，不重复改变状态、回复消费消息或启动任务。两个来源声称控制同一任务时，只停止可能重叠的写入，由 PM根据平台真实任务、工作目录和当前现场确认唯一 Worker；其他安全工作继续。
