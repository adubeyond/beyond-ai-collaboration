...
The shortest command above is sufficient for a same-directory, single-root project. Automatic discovery checks only the project root itself and exact Git roots that are direct children. Register every deeper or external formal Git root explicitly with `--repository-roots`. A cross-root or multi-repository project, or one that must support an existing platform worktree, must also obtain the actual host and Codex project identifiers from the current Codex project and pass `--host-id "<current hostId>" --codex-project-id "<current Codex projectId>"` during first fusion. Full verification refuses to call a multi-repository registration executable when those fields are absent. Later upgrades preserve the registered external roots and platform bindings; an absent path, non-exact Git root, or changed origin remote stops the update for an explicit decision instead of being silently rewritten.
