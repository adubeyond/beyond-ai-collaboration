...
本页用于最小示例体验。真实项目首次安装、版本升级、Hook审核、新/老项目初始化及可复制提示词统一见[安装、升级与项目初始化指南](../模板交付包/docs/安装升级与项目初始化指南.md)，不要从本页拼接一套生产接入顺序。
...
完成后，示例项目新增融合后的根`AGENTS.md`和项目级`.codex`身份护栏；已有Hook配置会被备份并合并，不会覆盖。BEYOND正式文档和项目登记位于项目内独立控制仓，个人工作台位于控制仓`local/`且不会进入 Git；项目根Git也会忽略`/beyond-control/`和`/.beyond-local-backups/`，避免把独立控制仓或本机备份误收为业务代码。项目总览和项目事实最初为空是预期状态，不要提前伪造结论。
