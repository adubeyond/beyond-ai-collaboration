...
安装完成后，先在Codex的Hook入口审核并信任BEYOND身份护栏，再从取得的不可变版本运行安装验真：
...
只有退出码为`0`并明确显示六个 Skill、控制仓结构、项目运行内核和身份护栏一致，才算内容安装完成。非零结果表示仍有旧文件、残留文件、弱入口、护栏缺失或项目入口没有融合，不能用模型自述替代。Hook首次出现或内容变化时只需信任一次，不是每个任务审批。验真通过后重启 Codex，再从示例项目根目录创建一个新任务。具体入口见[控制仓交付包说明](../模板交付包/README.md)。
