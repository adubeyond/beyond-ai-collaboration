...
- 有一个可写的本地目录。
- 如果要直接使用`$identity-pm`、`$identity-worker`和`$task-*`，需要先安装本仓库的六个Skills；只有当前客户端明确仍缓存旧Skill时才重启Codex。未安装时可以显式引用项目内对应的`SKILL.md`路径。
...
只有退出码为`0`并明确显示六个Skill、控制仓结构和项目运行内核一致，且没有BEYOND旧Hook残留，才算安装完成。`--content-only`只核对候选内容，不证明项目已经融合。验真通过后可从示例项目根目录创建新任务；只有当前客户端明确仍缓存旧Skill时才重启Codex。具体入口见[控制仓交付包说明](../模板交付包/README.md)。
...
3. 验真通过后创建新任务；只有当前客户端明确仍缓存旧Skill时才重启Codex。
4. 重新运行初始基线，再发送相同任务。
