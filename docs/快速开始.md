...
安装完成后，先从取得的不可变版本运行安装验真：

```text
node 模板交付包/scripts/verify-install-integrity.mjs --installed-skills-root "<Codex Skills目录>" --project-agents "<项目根>/AGENTS.md"
```

只有退出码为`0`并明确显示六个 Skill和项目通用入口一致，才算安装或升级完成。非零结果表示仍有旧文件、残留文件、内容差异或项目入口没有迁移，不能用模型自述替代。验真通过后重启 Codex，再从示例项目根目录创建一个新任务。具体 Skills 入口、编码要求和未安装时的显式路径用法见[模板交付包说明](../模板交付包/README.md)。
