...
> 首个公开版本为`v3.0.0-preview.1`。许可证、双语产品入口、公开贡献治理和干净候选Codex端到端验收已经闭合；跨平台安装、升级和回滚证据仍不完整，因此它被明确发布为Preview，而不是稳定版本。
...
把[模板交付包](模板交付包)中的内容复制到目标项目根目录。目标项目应至少得到：
...
将`skills/`中的六个 Skill 安装到当前 Codex 的 Skills 目录；如果当前环境尚未安装，可以在任务中显式引用对应的`SKILL.md`路径。安装后重新启动 Codex，使新任务加载当前版本。
...
这套系统当前围绕 Codex 的项目任务、Skills、正式任务线程和本地项目文档设计。不同平台是否支持正式任务创建、线程消息、派生实例和持久权限，需要按平台现实适配；平台能力缺失时不能伪造已经完成的控制关系。

当前Preview仍需要继续补齐跨平台安装、升级和回滚验证，并从公开远端持续复测快速开始。最终干净候选的本地静态门禁和Codex行为端到端验收已经通过，但这些证据只覆盖已声明的平台与任务契约，不能外推成所有环境均已稳定。

公开仓库创建后，还必须在 GitHub 仓库设置中启用`main`分支 ruleset，把“必须通过 PR、至少一名维护者批准、公开验证状态检查通过”设为平台强制门禁。仓库中的工作流和贡献指南不能代替这项服务端设置。
