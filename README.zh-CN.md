...
[快速开始](docs/快速开始.md) · [v3.0.3升级说明](docs/releases/v3.0.3.md) · [v3.0.4升级说明](docs/releases/v3.0.4.md) · [系统架构](docs/系统架构与运行机制.md) · [模板交付包](模板交付包) · [参与贡献](CONTRIBUTING.md)

> 当前公开版本为`v3.0.4`。`v3.0.2`已经废弃且从未进入公开主线；v3.0.4在保留生产级授权与证据边界的同时，进一步缩短了v3.0.3的真实运行路径。

## 1. 产品能力

BEYOND 通过以下机制管理真实项目中的 AI 工程任务：

- 项目总览保存稳定目标，任务台保存团队进度，项目事实保存技术栈、服务器和发布路径，不只依赖聊天上下文。
...
3. **项目化委派**：PM用六字段任务包明确结果、边界、验收、正式目标、事实入口和真实停止条件。
...
| **从阶段接力到任务自治** | 每完成一步就停下等待续派，普通测试失败也回到 PM | 六字段任务包一次授权；任务内切换动作；普通失败自动修复回测 | 任务能够从设计连续推进到正式交付或真实暂停 |
| **从默认信任到证据治理** | 开发自述被当成测试结论，测试通过被误认为可以发布 | 分离文件、Git、数据、环境和生产授权；建立一手证据、完成剖面与发布锚点 | 每个结论都能说明依据，权限不会沿流程静默扩张 |
| **从单线可用到多任务协同** | 多个任务同时写入容易串线和互相覆盖 | PM任务台管理负责人和进度；同一正式项目单 Worker写入；多人独立 clone用 Git协作 | PM 看得见团队，又不接管阶段执行 |
...
3. PM 用六字段任务包创建一个用户可见的正式任务线程。
4. 执行者成为唯一 Worker，读取当前动作需要的项目事实。
...
请形成六字段任务包并创建正式任务线程，由执行者连续完成需要的设计、开发、测试和运维动作。
```
...
- 同一正式项目同一时间只允许一个 Worker写入；其他任务可以并行调查，多人独立 clone时用 Git分支协作。
