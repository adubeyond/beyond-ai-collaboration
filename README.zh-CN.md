...
[快速开始](docs/快速开始.md) · [v3.1.0升级说明](docs/releases/v3.1.0.md) · [系统架构](docs/系统架构与运行机制.md) · [控制仓交付包](模板交付包) · [参与贡献](CONTRIBUTING.md)

> 当前版本为`v3.1.0`。它完整继承v3.0.9的身份、三态、正式Worker、PM退出和普通执行主链，只新增可选团队任务与协同、外置控制仓、新旧项目初始化引导和PM身份连续性护栏。

## 1. 产品能力

BEYOND 通过以下机制管理真实项目中的 AI 工程任务：

- 外置控制仓保存BEYOND文档和项目稳定事实，本机任务台保存个人进度；技术栈、服务器和发布路径不只依赖聊天上下文。
...
把[模板交付包](模板交付包)完整复制为与业务项目平级的`beyond-control`，不要把整套BEYOND文档复制到业务项目：
...
先运行`node beyond-control/scripts/beyond-control.mjs init-control`。新项目和已有项目都先显式进入`$identity-pm`，再分别输入`使用 BEYOND 初始化这个新项目。`或`使用 BEYOND 接入或升级这个已有项目。`已有项目先调查再逐组融合，不整目录覆盖，不自动删除原文档；该核对不进入后续普通任务路径。

本地入口融合不自动推送远端。固定脚本会建立共享项目登记、最小项目总览和最小事实索引，并在业务项目`.codex/`中合并身份护栏；已有Hook会先备份再保留。首次安装或护栏内容变化后，用户只需在Codex中审核并信任一次。需要让其他成员复用项目登记时，用户另行授权远端Git后，由PM用`project-registration`范围只推送同一项目的三份基础文件。

### 10.2 安装 Skills

将`beyond-control/skills/`中的六个 Skill安装到当前 Codex的 Skills目录；如果当前环境尚未安装，可以显式引用对应`SKILL.md`路径。完成项目入口与身份护栏融合、信任Hook并运行验真后，重新启动 Codex，使新任务加载当前版本。
...
空白项目会被引导补齐当前阶段确实需要的项目事实；已有项目会优先调查代码、配置、Git、测试和环境，不把可调查内容转成用户问卷。最低接入后只询问一次完整初始化或按需补齐。
...
| 路径 | 面向谁 | 用途 |
| --- | --- | --- |
| [模板交付包](模板交付包) | 模板使用者 | 可建立平级控制仓的正式机制、文档、共享结构、固定脚本和 Skills源码 |
