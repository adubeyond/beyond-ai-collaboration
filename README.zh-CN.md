...
[快速开始](docs/快速开始.md) · [v3.0.5升级说明](docs/releases/v3.0.5.md) · [v3.0.6升级说明](docs/releases/v3.0.6.md) · [系统架构](docs/系统架构与运行机制.md) · [模板交付包](模板交付包) · [参与贡献](CONTRIBUTING.md)

> 当前公开版本为`v3.0.6`。`v3.0.2`已经废弃且从未进入公开主线；v3.0.6继续缩短PM派单、Worker执行、结果回传和用户交付路径，并保留生产级授权、证据、Git和回滚能力。
...
3. **项目化委派**：PM用六字段任务包明确结果、边界、验收、正式目标、事实入口和真实停止条件；任务包只定义业务契约，不替Worker展开执行步骤。
...
3. PM 用六字段任务包创建一个用户可见的正式任务线程，首行显式加载 Worker身份和一个起始 Action Skill。
...
已有项目不要整目录覆盖。首次导入或升级时，保留现有项目事实，并一次性核对当前正式路径上的根/嵌套`AGENTS.md`、子项目入口和事实索引：保留项目边界与事实链接，迁出旧任务门禁、旧状态机和重复Skill规则。该核对不进入后续普通任务路径。
