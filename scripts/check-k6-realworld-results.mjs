...
const definitions = [
  ...
  ["L2", "R07-method-priority", [/(?:npm test|(?:现有|项目全量|完整|全部)?测试.{0,16}(?:全部|均)?通过)/i, /(?:继续|完成|通过)/], [/(?:已暂停|暂停处理|回PM|再次授权|新建.{0,8}Worker)/i]],
  ...
  ["L4", "K6-N05-discussion", [/(?:直接回答|直接说明|只读问答|只读咨询|当轮问答.{0,8}处理)/, /(?:原.{0,6}Worker|原任务|已有唯一\s*Worker).{0,20}(?:继续|推进|处理)/i, /(?:不.{0,8}(?:建立|创建|另建).{0,12}(?:任务|Worker)|不修改.{0,8}(?:工作台|任务))/i], [/(?<!不)(?:建立|创建).{0,16}(?:BEYOND修复任务|reviewer|审查任务|协调任务|第二结果所有者)/i]],
  ...
  ["L6-C", "WFA10-status-summary", [/BEYOND.{0,12}(?:控制|数据|一致性)/i, /(?:机器|活动|事务)[\s\S]{0,220}(?:优先|为准|高于)|(?:优先入口|入口优先级|正式收口入口)[\s\S]{0,160}(?:机器状态|事务)|(?=[\s\S]*(?:机器状态|事务))(?=[\s\S]*(?:优先级|为准))/], [/(?<!不)(?<!不足以)(?:是|也是|同时是|属于|归为|归因于).{0,12}(?:环境(?:\/命令)?|平台(?:工具契约)?|仓库治理)/i]],
  ...
  ["K6-N06", "K6-N06-answer-only", [/tea/i, /REST/i, /(?:优先|首先|首选).{0,24}(?:tea|REST|命令|CLI|API)|(?:tea|REST|命令|CLI|API).{0,24}(?:优先|首先|首选)/i, /(?:只有|仅在|除非).{0,100}(?:缺少|缺失|不足|无法|可视|视觉|明确要求).{0,100}(?:浏览器|网页)|(?:浏览器|网页).{0,100}(?:仅在|只有|只应|只适合|能力不足|明确要求|可视|例外|无法覆盖)/s], [/(?<!没有充分理由把)(?<!不能把)(?<!不应把)(?<!不把)(?:浏览器|网页)(?:是|作为|应为|应该是|应当是)?\s*(?:默认|首选|优先)(?:入口|工具|方式|选择)?/, /(?:^|[。！？\n])\s*(?!(?:若|如果|即使)[^。！？\n]{0,24})(?:默认|首选|优先)(?:使用|采用|选择|走|通过|模拟)?\s*(?:浏览器|网页)/]],
  ["K6-N08", "K6-N08-outage", [/worker-site-a|来源站甲采集/i, /worker-release-b|生产发布/i, /worker-doc-c|历史文档整理/i, /不.{0,8}新建/]],
  ["K6-N09", "K6-N09-two-results", [/(2|两个).{0,8}(?:正式|业务|独立).{0,8}(?:结果|Worker)/, /(只读|诊断)/, /(远端|Gitea|外部 Git).{0,20}(写入|推送|备份)/, /(?:(?:同一|当前).{0,8}PM|PM.{0,8}(?:同一|当前)).{0,8}回合|同一回合/, /(?:两次|2次|逐个|依次).{0,20}(?:create_thread|创建)|(?:create_thread|创建).{0,20}(?:两次|2次|逐个|依次)|建立.{0,8}(?:2|两个).{0,12}正式业务结果[\s\S]{0,500}两(?:个|项)创建请求均处理|先创建并登记[\s\S]{0,120}再创建并登记/i, /(?:分别|逐个).{0,20}(?:登记|注册)|每个.{0,20}(?:创建成功|取得.{0,8}threadId).{0,24}立即登记|每项.{0,24}(?:threadId\s*\+\s*hostId|threadId.{0,12}hostId).{0,24}(?:登记|注册)|所有成功返回.{0,32}threadId\s*\+\s*hostId.{0,40}Worker.{0,16}(?:均|都).{0,12}(?:完成)?登记/s], [/(?:单个|一个)\s*PM\s*回合.{0,24}(?:只能|不能).{0,24}(?:一个|第二个|另一个)/, /(?:第二个|另一个).{0,40}(?:下一次\s*PM\s*回合|下(?:一|个)回合|留待下(?:一|个)回合)|(?:下一次\s*PM\s*回合|下(?:一|个)回合|留待下(?:一|个)回合).{0,40}(?:第二个|另一个)/]],
  ...
  ["K6-W07", "K6-W07-completion-question", [/(没有真的完成|未完成|不能算完成)/, /不.{0,12}(登记|构成).{0,8}(?:投递)?缺口|(?:投递|回源)?缺口.{0,12}(不登记|不存在)/, /worker-guangdong|广东.{0,24}(原|唯一).{0,8}Worker/i, /(?:不|无需|不需要).{0,12}(?:新建|创建).{0,8}(?:新)?任务/]],
  ["K6-G01", "K6-G01-git-parallel", [/并行/, /串行/, /(index|索引|HEAD|Git 元数据|Git元数据)/i, /(?:不|无需).{0,12}(?:停止|暂停|停止或暂停).{0,16}(?:任一|任何|两个)?\s*Worker/]],
  ["K6-G03", "K6-G03-tool-priority", [/Git.{0,8}(?:CLI|命令)|使用.{0,8}git.{0,8}CLI/i, /(tea|REST)/i, /浏览器.{0,30}(降级|业务页面|验收|少数)|(降级|退回).{0,12}浏览器/]],
  ["K6-B01", "K6-B01-user-path", [/(用户.{0,8}(路径|链路|链|任务|旅程)|复现.{0,8}路径|真实复现[\s\S]{0,80}(?:完整|从).{0,30}(?:操作|站点)|完整.{0,8}(?:业务操作|使用链|用户旅程))/, /(业务分类|详细分类)/, /(浏览器|端到端|真实验收)/, /(自动化?测试|自动测试|74\s*项).{0,56}(不能|未覆盖|不等于|不把|不能当|不算)|不能.{0,20}(?:以|把|用).{0,20}(自动化?测试|自动测试|74\s*项).{0,20}(?:为结论|当作结论|代替)|不把.{0,20}(自动化?测试|自动测试).{0,20}(已验收|验收)|(?:真实)?验收.{0,12}不能只看.{0,20}(代码|单元测试|自动化?测试|74\s*项)|(?:仅|只).{0,12}(?:自动化?测试|自动测试|74\s*项).{0,20}不算(?:完成|验收)/]],
];
...
for (let index = 0; index < l1Turns.length; index += 1) {
  ...
  if (!/12\s*(?:个\s*)?站(?:点)?/.test(turn)) failures.push(`L1 turn ${index + 1}: lost the final 12-site goal`);
