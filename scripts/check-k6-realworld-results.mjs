...
const definitions = [
  ...
  ["K6-B01", "K6-B01-user-path", [/(用户.{0,8}路径|复现.{0,8}路径)/, /(业务分类|详细分类)/, /(浏览器|端到端|真实验收)/, /(自动化?测试|自动测试|74\s*项).{0,40}(不能|未覆盖|不等于|不把|不能当)|不把.{0,20}(自动化?测试|自动测试).{0,20}(已验收|验收)|(?:真实)?验收.{0,12}不能只看.{0,20}(代码|单元测试|自动化?测试|74\s*项)/]],
];
