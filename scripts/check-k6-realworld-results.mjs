...
const definitions = [
  ["K6-N03", "K6-N03-approval", [/worker-site-a/i, /终止公告/, /(?:不生效|不包含|未授权|不启动|不执行)[\s\S]{0,100}(?:定时|周期)|(?:定时|周期).{0,80}(?:不生效|不包含|未授权|不启动|不执行)/, /(?:不生效|不包含|未授权|不执行)[\s\S]{0,120}(?:生产发布|发布到生产)|(?:生产发布|发布到生产).{0,80}(?:不生效|不包含|未授权|不执行)/]],
  ["K6-N05", "K6-N05-discussion", [/Gitea/i, /(CLI|命令)/i, /API/i, /(没有授权|(没有|未)(创建|派发|修改)|不.{0,8}(创建|派发|修改))/]],
  ...
  ["K6-N09", "K6-N09-two-results", [/(2|两个).{0,8}(正式|业务).{0,8}结果/, /(只读|诊断)/, /(远端|Gitea).{0,20}写入/]],
  ["K6-N10", "K6-N10-temporary-model", [/当前 PM 对话|当前PM对话/i, /不.{0,12}工作台/, /不影响[\s\S]{0,140}AGENTS\.md|不应写入[\s\S]{0,80}AGENTS\.md/i, /(已经|既有|现有).{0,12}Worker/i, /失效/]],
  ["K6-W02", "K6-W02-stage-progress", [/进行中/, /worker-guangdong|当前.{0,12}唯一 Worker/i, /(?:不回|无需回|不需要回).{0,8}PM|PM.{0,12}(?:不裁决|无需裁决)/i, /(?:不需要|无需).{0,12}(?:再次|重新)?授权|(?:再次|重新)授权.{0,12}(?:不需要|无需)/]],
  ["K6-W04", "K6-W04-background-job", [/(不.{0,8}跟着|不.{0,8}高频|不.{0,8}轮询)/, /(并行|准备).{0,20}(切流|验收|回滚|缓存)/, /PM.{0,30}(?:不接管|不逐步|不要求|不需要|无需|不应)/]],
  ["K6-W07", "K6-W07-completion-question", [/(没有真的完成|未完成|不能算完成)/, /不.{0,12}(登记|构成).{0,8}回源缺口|回源缺口.{0,12}(不登记|不存在)/, /worker-guangdong/i, /不.{0,8}新建/]],
  ...
  ["K6-B01", "K6-B01-user-path", [/(用户.{0,8}路径|复现路径)/, /(业务分类|详细分类)/, /(浏览器|端到端|真实验收)/, /(自动化?测试|74 项).{0,40}(不能|未覆盖|不等于)|验收不能只看.{0,20}(代码|单元测试)/]],
];
