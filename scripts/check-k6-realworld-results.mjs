...
const definitions = [
  ["K6-N03", "K6-N03-approval", [/worker-site-a/i, /终止公告/, /(?:不生效|不包含|未授权|不是授权|不是批准|不启动|不执行)[\s\S]{0,100}(?:定时|周期)|(?:定时|周期).{0,100}(?:不生效|不包含|未授权|不是授权|不是批准|不启动|不执行|不是.{0,12}批准事项)/, /(?:不生效|不包含|未授权|不是授权|不是批准|不执行)[\s\S]{0,120}(?:生产发布|发布到生产|发布生产)|(?:生产发布|发布到生产|发布生产).{0,100}(?:不生效|不包含|未授权|不是授权|不是批准|不执行|不是.{0,12}批准事项)/]],
  ...
  ["K6-W02", "K6-W02-stage-progress", [/进行中/, /worker-guangdong|当前.{0,12}唯一 Worker/i, /(?:不回|无需回|不需要回).{0,8}PM|PM.{0,20}(?:不裁决|无需裁决|不需要|无需)/i, /(?:不需要|无需).{0,12}(?:再次|重新)?授权|(?:再次|重新)授权.{0,12}(?:不需要|无需)/]],
  ...
  ["K6-B01", "K6-B01-user-path", [/(用户.{0,8}路径|复现.{0,8}路径)/, /(业务分类|详细分类)/, /(浏览器|端到端|真实验收)/, /(自动化?测试|74\s*项).{0,40}(不能|未覆盖|不等于)|(?:真实)?验收.{0,12}不能只看.{0,20}(代码|单元测试|自动化?测试|74\s*项)/]],
];
