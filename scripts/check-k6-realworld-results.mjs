...
const definitions = [
  ["K6-N03", "K6-N03-approval", [/worker-site-a/i, /终止公告/, /(?:不生效|不包含|未授权|未获批准|不是授权|不是批准|不等于批准|不构成.{0,12}授权|不启动|不执行)[\s\S]{0,100}(?:定时|周期)|(?:定时|周期).{0,100}(?:不生效|不包含|未授权|未获批准|不是授权|不是批准|不等于批准|不构成.{0,12}授权|不启动|不执行|不是.{0,12}批准事项)/, /(?:不生效|不包含|未授权|未获批准|不是授权|不是批准|不等于批准|不构成.{0,12}授权|不执行)[\s\S]{0,120}(?:生产发布|发布到生产|发布生产)|(?:生产发布|发布到生产|发布生产).{0,100}(?:不生效|不包含|未授权|未获批准|不是授权|不是批准|不等于批准|不构成.{0,12}授权|不执行|不是.{0,12}批准事项)/]],
  ...
  ["K6-N08", "K6-N08-outage", [/worker-site-a/i, /worker-release-b|生产发布/i, /worker-doc-c|历史文档整理/i, /不.{0,8}新建/]],
  ["K6-N09", "K6-N09-two-results", [/(2|两个).{0,8}(正式|业务).{0,8}结果/, /(只读|诊断)/, /(远端|Gitea|外部 Git).{0,20}(写入|推送|备份)/]],
  ["K6-N10", "K6-N10-temporary-model", [/当前 PM 对话|当前PM对话/i, /不.{0,12}工作台/, /(?:不会|不应|不得|不).{0,20}(?:根入口|根规则|AGENTS\.md|其他文件)|(?:根入口|根规则|AGENTS\.md|其他文件).{0,20}(?:不会|不应|不得|不)(?:写入)?/i, /(已经|既有|现有|已存在).{0,12}Worker/i, /失效/]],
  ["K6-W02", "K6-W02-stage-progress", [/进行中/, /worker-guangdong|当前.{0,20}(?:唯一|原) Worker|当前对话这个唯一、原 Worker/i, /(?:不回|无需回|不需要回|不切给).{0,8}PM|PM.{0,20}(?:不裁决|无需裁决|不需要|无需)/i, /(?:不需要|无需).{0,12}(?:再次|重新)?授权|(?:再次|重新)授权.{0,12}(?:不需要|无需)/]],
  ...
  ["K6-B01", "K6-B01-user-path", [/(用户.{0,8}(路径|链路)|复现.{0,8}路径|真实复现.{0,8}(整体操作|操作链)|完整业务操作|完整使用链)/, /(业务分类|详细分类)/, /(浏览器|端到端|真实验收)/, /(自动化?测试|自动测试|74\s*项).{0,40}(不能|未覆盖|不等于|不把|不能当|不算)|不把.{0,20}(自动化?测试|自动测试).{0,20}(已验收|验收)|(?:真实)?验收.{0,12}不能只看.{0,20}(代码|单元测试|自动化?测试|74\s*项)|(?:仅|只).{0,12}(?:自动化?测试|自动测试|74\s*项).{0,20}不算(?:完成|验收)/]],
];
