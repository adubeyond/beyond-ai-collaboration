...
check('WFA-03 separates source site from business type', /北大荒.{0,80}(不是|不等于|不能当成).{0,24}(七类|业务|公告业务类型)|(?:七类|业务|公告类型).{0,40}(不包含|不是|不应该).{0,24}(北大荒|来源站|站点名称)/s.test(message));
...
check('WFA-03 keeps storage fields distinct', ['sourceSite', 'sourceSection', 'noticeType', 'displayChannel'].every((field) => message.includes(field)));
